...
-- Ledger de pagamento de comissão por destinatário do rateio (18/09/2026) —
-- substitui o flag booleano comissao_paga em fechamento_negocios (colunas
-- ficam no lugar, sem migração destrutiva, só param de ser lidas/escritas).
-- Um negócio pode ter vários destinatários (levantamento/fechamento/captação,
-- pessoas diferentes) pagos em momentos e valores diferentes — por isso o
-- ledger é por linha de rateio (fechamento_negocio_corretores), não por
-- negócio.
...
CREATE INDEX IF NOT EXISTS idx_fpag_negcorr ON fechamento_pagamentos(negocio_corretor_id);
